...
#define LINPHONE_IOS_VERSION "4.2-93-g91b0826"
...
@"LinphoneNotifyPresenceReceivedForUriOrTel";
...
const int kLinphoneAudioVbrCodecDefaultBitrate =
36; /*you can override this from linphonerc or linphonerc-factory*/
...
"AV Capture: com.apple.avfoundation.avcapturedevice.built-in_video:1" /*"AV Capture: Front     \
Camera"*/
#define BACK_CAM_NAME                                                                              \
"AV Capture: com.apple.avfoundation.avcapturedevice.built-in_video:0" /*"AV Capture: Back      \
Camera"*/
...
+ (NSSet *)unsupportedCodecs {
    ...
    for (int i = 0; codec_pref_table[i].name != NULL; ++i) {
        PayloadType *available = linphone_core_find_payload_type(
                                                                 theLinphoneCore, codec_pref_table[i].name, codec_pref_table[i].rate,
...
+ (LinphoneManager *)instance {
    @synchronized(self) {
        if (theLinphoneManager == nil) { theLinphoneManager = [[LinphoneManager alloc] init]; }
    }
...
+ (void)instanceRelease {
    if (theLinphoneManager != nil) { theLinphoneManager = nil; }
}
#endif
...
+ (BOOL)langageDirectionIsRTL {
    ...
    dispatch_once(&onceToken, ^{
        dir = [NSLocale characterDirectionForLanguage:[[NSLocale currentLocale]
...
- (id)init {
    if ((self = [super init])) {
        ...
        self.messagePlayer =
        [[AVAudioPlayer alloc] initWithContentsOfURL:[NSURL URLWithString:path] error:nil];
        ...
        // by default if handle_content_encoding is not set, we use plain text for debug purposes
        // only
...
#else
            [self lpConfigSetString:@"conflate"
                             forKey:@"handle_content_encoding"
                          inSection:@"misc"];
#endif
        }
...
- (void)migrationAllPre {
    ...
    [self lpConfigSetBool:NO
                   forKey:@"store_friends"
                inSection:@"misc"]; // so far, storing friends in files is not needed. may change in
    // the future.
}
...
- (BOOL)migrateChatDBIfNeeded:(LinphoneCore *)lc {
    ...
    const char *check_appdata = "SELECT url,message FROM history WHERE url LIKE 'assets-library%' "
    "OR message LIKE 'assets-library%' LIMIT 1;";
    ...
    if (shouldMigrate) {
        ...
        // migrate old chats to the new db. The iOS stores timestamp in UTC already, so we can
        // directly put it in the 'utc' field and set 'time' to -1
        const char *migration_statement =
        "INSERT INTO history "
        "(localContact,remoteContact,direction,message,utc,read,status,time) "
        "SELECT localContact,remoteContact,direction,message,time,read,state,'-1' FROM "
        "oldchats.chat";
        ...
        if (default_proxy) { identity = linphone_proxy_config_get_identity(default_proxy); }
        if (!identity) { identity = "sip:unknown@sip.linphone.org"; }
        
        char *from_conversion = sqlite3_mprintf(
                                                "UPDATE history SET localContact = %Q WHERE localContact = ''", identity);
        ...
        char *to_conversion = sqlite3_mprintf(
                                              "UPDATE history SET remoteContact = %Q WHERE remoteContact = ''", identity);
    ...
    const char *assetslib_migration =
    "UPDATE history SET appdata='{\"localimage\":\"'||message||'\"}' , message='' "
    ...
    const char *assetslib_migration_fromurl = "UPDATE history SET "
    "appdata='{\"localimage\":\"'||url||'\"}' , url='' "
    "WHERE url LIKE 'assets-library%'";
    ...
    // We will lose received messages with remote url, they will be displayed in plain. We can't do
    // much for them..
    migrated = TRUE;
...
- (void)migrateFromUserPrefs {
    ...
    if (_configDb == nil) return;
    
    if ([self lpConfigIntForKey:migration_flag withDefault:0]) { return; }
    ...
    for (NSString *userpref in values) {
        if ([defaults_keys containsObject:userpref]) {
            LOGI(@"Migrating %@ from user preferences: %d", userpref,
                 [[defaults objectForKey:userpref] boolValue]);
...
- (void)migrationLinphoneSettings {
    // we need to proceed to the migration *after* the chat database was opened, so that we know it
    ...
    if ([self lpConfigBoolForKey:@"file_transfer_migration_done"] == FALSE) {
        ...
        LOGI(@"Migrating sharing server url from %s to %s",
             linphone_core_get_file_transfer_server(LC), newURL);
...
static void migrateWizardToAssistant(const char *entry, void *user_data) {
    ...
    [thiz lpConfigSetString:[thiz lpConfigStringForKey:key inSection:@"wizard"]
                     forKey:key
                  inSection:@"assistant"];
}
...
static void linphone_iphone_display_status(struct _LinphoneCore *lc, const char *message) {
    NSString *status =
    [[NSString alloc] initWithCString:message encoding:[NSString defaultCStringEncoding]];
    [(__bridge LinphoneManager *)linphone_core_cbs_get_user_data(
                                                                 linphone_core_get_current_callbacks(lc)) displayStatus:status];
}
...
- (void)userNotifContinue:(NSTimer *)timer {
    ...
    if (content && [UIApplication sharedApplication].applicationState != UIApplicationStateActive) {
        ...
        UNNotificationRequest *req = [UNNotificationRequest requestWithIdentifier:@"call_request"
                                                                          content:content
                                                                          trigger:NULL];
...
- (void)onCall:(LinphoneCall *)call
  StateChanged:(LinphoneCallState)state
...
#pragma deploymate pop
    ...
    if (state == LinphoneCallIncomingReceived) {
        ...
        LOGI(@"Decrementing index of long running task for call id : %@ with index : %d", callId,
             index);
        ...
        if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_9_x_Max) {
            if (call && (linphone_core_get_calls_nb(LC) < 2)) {
                ...
                NSString *callId =
                [NSString stringWithUTF8String:linphone_call_log_get_call_id(
                ...
                BOOL video = FALSE;
                video      = ([UIApplication sharedApplication].applicationState ==
                              UIApplicationStateActive &&
                              linphone_core_get_video_policy(LC)->automatically_accept &&
                              linphone_call_params_video_enabled(linphone_call_get_remote_params(call)));
                [LinphoneManager.instance.providerDelegate reportIncomingCall:call
                                                                     withUUID:uuid
                                                                       handle:address
                                                                        video:video];
#else
...
#endif
            } else if ([UIApplication sharedApplication].applicationState !=
                       UIApplicationStateActive) {
                ...
                [UNNotificationRequest requestWithIdentifier:@"call_request"
                                                     content:content
                                                     trigger:NULL];
                [[UNUserNotificationCenter currentNotificationCenter]
                 addNotificationRequest:req
            ...
            if ([UIApplication sharedApplication].applicationState != UIApplicationStateActive) {
                ...
                if (floor(NSFoundationVersionNumber) <= NSFoundationVersionNumber_iOS_9_x_Max) {
                    ...
                    UIMutableUserNotificationCategory *callcat =
                    [[UIMutableUserNotificationCategory alloc] init];
                    callcat.identifier = @"incoming_call";
                    [callcat setActions:callactions
                    ...
                    if (data->notification) {
                        ...
                        if ([[UIDevice currentDevice].systemVersion floatValue] >= 8 &&
                            ...
                            NSString *ring =
                            ([LinphoneManager
                              bundleFile:[self lpConfigStringForKey:@"local_ring"
                                                          inSection:@"sound"]
                              .lastPathComponent]
                        ...
                        @{ @"callId" : callId,
                           @"timer" : [NSNumber numberWithInt:1] };
                        ...
                        if (!incallBgTask) {
                            incallBgTask = [[UIApplication sharedApplication]
                                            beginBackgroundTaskWithExpirationHandler:^{
                                                LOGW(@"Call cannot ring any more, too late");
                                                [[UIApplication sharedApplication]
                                                 endBackgroundTask:incallBgTask];
                                                incallBgTask = 0;
                                            }];
                            
                            if (data->timer) {
                                [[NSRunLoop currentRunLoop] addTimer:data->timer
            ...
            if (uuid) {
                ...
                if (callKit_callId && !_conf) {
                    ...
                    [LinphoneManager.instance.providerDelegate.uuids setObject:callKit_uuid
                                                                        forKey:callKit_callIdNS];
                    [LinphoneManager.instance.providerDelegate.calls setObject:callKit_callIdNS
                                                                        forKey:callKit_uuid];
                    NSString *address = [FastAddressBook
                                         displayNameForAddress:linphone_call_get_remote_address(callKit_call)];
                    CXHandle *handle =
                    [[CXHandle alloc] initWithType:CXHandleTypeGeneric value:address];
                    CXStartCallAction *act =
                    [[CXStartCallAction alloc] initWithCallUUID:callKit_uuid handle:handle];
    ...
    if (state == LinphoneCallIncomingReceived || state == LinphoneCallOutgoingInit ||
        ...
        if (linphone_call_params_video_enabled(linphone_call_get_current_params(call)) &&
            !speaker_already_enabled && !_bluetoothEnabled) {
    ...
    if (state == LinphoneCallConnected && !mCallCenter) {
        ...
        [[UIDevice currentDevice]
         setProximityMonitoringEnabled:!(_speakerEnabled || _bluetoothEnabled)];
    }
...
static void linphone_iphone_call_state(LinphoneCore *lc, LinphoneCall *call,
    ...
    [(__bridge LinphoneManager *)linphone_core_cbs_get_user_data(
                                                                 linphone_core_get_current_callbacks(lc)) onCall:call
     StateChanged:state
     withMessage:message];
}
...
static void linphone_iphone_transfer_state_changed(LinphoneCore *lc, LinphoneCall *call,
                                                   LinphoneCallState state) {}

#pragma mark - Global state change

static void linphone_iphone_global_state_changed(LinphoneCore *lc, LinphoneGlobalState gstate,
                                                 const char *message) {
    [(__bridge LinphoneManager *)linphone_core_cbs_get_user_data(
                                                                 linphone_core_get_current_callbacks(lc)) onGlobalStateChanged:gstate
     withMessage:message];
}
...
- (void)onGlobalStateChanged:(LinphoneGlobalState)state withMessage:(const char *)message {
    ...
    dispatch_async(dispatch_get_main_queue(), ^(void) {
        [NSNotificationCenter.defaultCenter postNotificationName:kLinphoneGlobalStateUpdate
                                                          object:self
                                                        userInfo:dict];
    });
}
...
- (void)globalStateChangedNotificationHandler:(NSNotification *)notif {
    if ((LinphoneGlobalState)[[[notif userInfo] valueForKey:@"state"] integerValue] ==
        LinphoneGlobalOn) {
...
static void linphone_iphone_configuring_status_changed(LinphoneCore *lc,
                                                       LinphoneConfiguringState status,
                                                       const char *message) {
    [(__bridge LinphoneManager *)linphone_core_cbs_get_user_data(
                                                                 linphone_core_get_current_callbacks(lc)) onConfiguringStatusChanged:status
     withMessage:message];
}

- (void)onConfiguringStatusChanged:(LinphoneConfiguringState)status
                       withMessage:(const char *)message {
...
- (void)configuringStateChangedNotificationHandler:(NSNotification *)notif {
    _wasRemoteProvisioned =
    ((LinphoneConfiguringState)[[[notif userInfo] valueForKey:@"state"] integerValue] ==
     LinphoneConfiguringSuccessful);
    if (_wasRemoteProvisioned) {
        LinphoneProxyConfig *cfg = linphone_core_get_default_proxy_config(LC);
        if (cfg) { [self configurePushTokenForProxyConfig:cfg]; }
    }
}
...
- (void)onRegister:(LinphoneCore *)lc
    ...
    LOGI(@"New registration state: %s (message: %s)", linphone_registration_state_to_string(state),
         cmessage);
    ...
    switch (reason) {
        ...
        case LinphoneReasonIOError:
            message = NSLocalizedString(@"Cannot reach the server: either it is an invalid address "
                                        @"or it may be temporary down.",
                                        nil);
            break;
            
        case LinphoneReasonUnauthorized:
            message =
            NSLocalizedString(@"Operation is unauthorized because missing credential", nil);
            break;
        ...
        case LinphoneReasonNotAcceptable:
            message = NSLocalizedString(@"NotAcceptable", nil);
            break;
        case LinphoneReasonDoNotDisturb:
            message = NSLocalizedString(@"DoNotDisturb", nil);
            break;
        case LinphoneReasonDeclined:
            message = NSLocalizedString(@"Declined", nil);
            break;
        case LinphoneReasonNotFound:
            message = NSLocalizedString(@"NotFound", nil);
            break;
        case LinphoneReasonNotAnswered:
            message = NSLocalizedString(@"NotAnswered", nil);
            break;
        case LinphoneReasonBusy:
            message = NSLocalizedString(@"Busy", nil);
            break;
    ...
    [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:state], @"state",
     [NSValue valueWithPointer:cfg], @"cfg", message,
     @"message", nil];
    [NSNotificationCenter.defaultCenter postNotificationName:kLinphoneRegistrationUpdate
                                                      object:self
                                                    userInfo:dict];
}
...
static void linphone_iphone_registration_state(LinphoneCore *lc, LinphoneProxyConfig *cfg,
                                               ...
                                               const char *message) {
    [(__bridge LinphoneManager *)linphone_core_cbs_get_user_data(
                                                                 linphone_core_get_current_callbacks(lc)) onRegister:lc
     cfg:cfg
     state:state
     message:message];
}
...
static void linphone_iphone_popup_password_request(LinphoneCore *lc, LinphoneAuthInfo *auth_info,
    ...
    //        }
    //
    //        // dont pop up if we are in background, in any case we will refresh registers when
    //        entering
    ...
    //        alertControllerWithTitle:NSLocalizedString(@"Authentification needed", nil)
    //                                                        message:[NSString
    //                                                        stringWithFormat:NSLocalizedString(@"Registration
    //                                                        failed because authentication is "
    ...
    //        actionWithTitle:NSLocalizedString(@"Confirm password", nil)
    //                                                                 style:UIAlertActionStyleDefault
    //                                                               handler:^(UIAlertAction *
    //                                                               action) {
    //                                                                   NSString *password =
    ...
    //                                                                   linphone_auth_info_new(username.UTF8String,
    //                                                                   NULL, password.UTF8String,
    //                                                                   NULL,
    //                                                                                          realm.UTF8String,
    //                                                                                          domain.UTF8String);
    //                                                                   linphone_core_add_auth_info(LC,
    //                                                                   info);
    //                                                                   [LinphoneManager.instance
    //                                                                   refreshRegisters];
    //                                                               }];
    //
    //        UIAlertAction* settingsAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"Go
    //        to settings", nil)
    //                                                                 style:UIAlertActionStyleDefault
    //                                                               handler:^(UIAlertAction *
    //                                                               action) {
...
- (void)onMessageReceived:(LinphoneCore *)lc
                     room:(LinphoneChatRoom *)room
                  message:(LinphoneChatMessage *)msg {
#pragma deploymate push "ignored-api-availability"
    if (_silentPushCompletion) {
        // we were woken up by a silent push. Call the completion handler with NEWDATA
        // so that the push is notified to the user
...
#pragma deploymate pop
    NSString *callID =
    [NSString stringWithUTF8String:linphone_chat_message_get_custom_header(msg, "Call-ID")];
    ...
    if (linphone_chat_message_is_file_transfer(msg) || linphone_chat_message_is_text(msg)) {
        //        if ([UIApplication sharedApplication].applicationState != UIApplicationStateActive
        //        ||
        //            ((PhoneMainView.instance.currentView !=
        //            ChatsListView.compositeViewDescription) &&
        //             ((PhoneMainView.instance.currentView !=
        //             ChatConversationView.compositeViewDescription))) ||
        //            (PhoneMainView.instance.currentView ==
        //            ChatConversationView.compositeViewDescription &&
        ...
        //                    [LinphoneManager.instance lpConfigBoolForKey:@"show_msg_in_notif"] ==
        //                    NO) {
        //
        //                    UIMutableUserNotificationAction *reply =
        //                    [[UIMutableUserNotificationAction alloc] init]; reply.identifier =
        //                    @"reply"; reply.title = NSLocalizedString(@"Reply", nil);
        //                    reply.activationMode = UIUserNotificationActivationModeForeground;
        //                    reply.destructive = NO;
        //                    reply.authenticationRequired = YES;
        //
        //                    UIMutableUserNotificationAction *mark_read =
        //                    [[UIMutableUserNotificationAction alloc] init]; mark_read.identifier =
        //                    @"mark_read"; mark_read.title = NSLocalizedString(@"Mark Read", nil);
        ...
        //                } else {
        //                    // iOS 9 allows for inline reply. We don't propose mark_read in this
        //                    case UIMutableUserNotificationAction *reply_inline =
        //                    [[UIMutableUserNotificationAction alloc] init];
        //
        //                    reply_inline.identifier = @"reply_inline";
        //                    reply_inline.title = NSLocalizedString(@"Reply", nil);
        //                    reply_inline.activationMode =
        //                    UIUserNotificationActivationModeBackground; reply_inline.destructive =
        //                    NO; reply_inline.authenticationRequired = NO; reply_inline.behavior =
        //                    UIUserNotificationActionBehaviorTextInput;
        //
        //                    actions = @[ reply_inline ];
        //                }
        //
        //                UIMutableUserNotificationCategory *msgcat =
        //                [[UIMutableUserNotificationCategory alloc] init]; msgcat.identifier =
        //                @"incoming_msg"; [msgcat setActions:actions
        //                forContext:UIUserNotificationActionContextDefault]; [msgcat
        //                setActions:actions forContext:UIUserNotificationActionContextMinimal];
        //
        ...
        //                        stringWithFormat:NSLocalizedString(@"IM_FULLMSG", nil), from,
        //                        chat];
        //                    } else {
        //                        notif.alertBody = [NSString
        //                        stringWithFormat:NSLocalizedString(@"IM_MSG", nil), from];
        //                    }
        //                    notif.alertAction = NSLocalizedString(@"Show", nil);
        //                    notif.soundName = @"msg.caf";
        //                    notif.userInfo = @{ @"from" : from, @"from_addr" : remote_uri,
        //                    @"call-id" : callID }; notif.accessibilityLabel = @"Message notif";
        //                    [[UIApplication sharedApplication] presentLocalNotificationNow:notif];
        //                }
        //            } else {
        //                UNMutableNotificationContent *content = [[UNMutableNotificationContent
        //                alloc] init]; content.title = NSLocalizedString(@"Message received", nil);
        //                if ([LinphoneManager.instance lpConfigBoolForKey:@"show_msg_in_notif"
        //                withDefault:YES]) {
        ...
        //                content.userInfo = @{ @"from" : from, @"from_addr" : remote_uri, @"CallId"
        //                : callID }; content.accessibilityLabel = @"Message notif";
        //                UNNotificationRequest *req =
        //                    [UNNotificationRequest requestWithIdentifier:@"call_request"
        //                    content:content trigger:NULL];
        //                [[UNUserNotificationCenter currentNotificationCenter]
        //                    addNotificationRequest:req
        //                     withCompletionHandler:^(NSError *_Nullable error) {
        //                       // Enable or disable features based on authorization.
        ...
        NSDictionary *dict = @{
                               ...
                               @"from_address" :
                                   [NSValue valueWithPointer:linphone_chat_message_get_from_address(msg)],
...
static void linphone_iphone_message_received(LinphoneCore *lc, LinphoneChatRoom *room,
                                             LinphoneChatMessage *message) {
    [(__bridge LinphoneManager *)linphone_core_cbs_get_user_data(
                                                                 linphone_core_get_current_callbacks(lc)) onMessageReceived:lc
     room:room
     message:message];
}

static void linphone_iphone_message_received_unable_decrypt(LinphoneCore *lc,
                                                            LinphoneChatRoom *room,
                                                            LinphoneChatMessage *message) {
    
    NSString *msgId =
    [NSString stringWithUTF8String:linphone_chat_message_get_custom_header(message, "Call-ID")];
    ...
    if ([UIApplication sharedApplication].applicationState != UIApplicationStateActive) {
        if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_9_x_Max) {
            ...
            content.body                          = body;
            UNNotificationRequest *req =
            [UNNotificationRequest requestWithIdentifier:@"decrypt_request"
                                                 content:content
                                                 trigger:NULL];
            [[UNUserNotificationCenter currentNotificationCenter]
             addNotificationRequest:req
        ...
        UIAlertAction *callAction = [UIAlertAction
                                     ...
                                     style:UIAlertActionStyleDefault
                                     handler:^(UIAlertAction *action) { [LinphoneManager.instance call:address]; }];
        
        [errView addAction:defaultAction];
        [errView addAction:callAction];
        //        [PhoneMainView.instance presentViewController:errView animated:YES
        //        completion:nil];
    }
}
...
- (void)onNotifyReceived:(LinphoneCore *)lc
    ...
    if (body != NULL) { [dict setObject:[NSValue valueWithPointer:body] forKey:@"content"]; }
    [NSNotificationCenter.defaultCenter postNotificationName:kLinphoneNotifyReceived
                                                      object:self
                                                    userInfo:dict];
}

static void linphone_iphone_notify_received(LinphoneCore *lc, LinphoneEvent *lev,
                                            const char *notified_event,
                                            const LinphoneContent *body) {
    [(__bridge LinphoneManager *)linphone_core_cbs_get_user_data(
                                                                 linphone_core_get_current_callbacks(lc)) onNotifyReceived:lc
     event:lev
...
- (void)onNotifyPresenceReceivedForUriOrTel:(LinphoneCore *)lc
    ...
    [NSNotificationCenter.defaultCenter
     postNotificationName:kLinphoneNotifyPresenceReceivedForUriOrTel
     object:self
     userInfo:dict];
}

static void linphone_iphone_notify_presence_received_for_uri_or_tel(
                                                                    LinphoneCore *lc, LinphoneFriend *lf, const char *uri_or_tel,
                                                                    const LinphonePresenceModel *presence_model) {
    [(__bridge LinphoneManager *)linphone_core_cbs_get_user_data(
                                                                 linphone_core_get_current_callbacks(lc))
     onNotifyPresenceReceivedForUriOrTel:lc
     friend:lf
     uri:uri_or_tel
...
static void linphone_iphone_call_encryption_changed(LinphoneCore *lc, LinphoneCall *call, bool_t on,
    ...
    [(__bridge LinphoneManager *)linphone_core_cbs_get_user_data(
                                                                 linphone_core_get_current_callbacks(lc)) onCallEncryptionChanged:lc
     call:call
     on:on
...
- (void)alertLIME:(LinphoneChatRoom *)room {
    ...
    if ([UIApplication sharedApplication].applicationState == UIApplicationStateActive) {
        ...
        [UIAlertAction actionWithTitle:NSLocalizedString(@"Call", nil)
                                 style:UIAlertActionStyleDefault
                               handler:^(UIAlertAction *action) {
        ...
        if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_9_x_Max) {
            ...
            UNNotificationRequest *req =
            [UNNotificationRequest requestWithIdentifier:@"lime_request"
                                                 content:content
...
static void linphone_iphone_is_composing_received(LinphoneCore *lc, LinphoneChatRoom *room) {
    [(__bridge LinphoneManager *)linphone_core_cbs_get_user_data(
                                                                 linphone_core_get_current_callbacks(lc)) onMessageComposeReceived:lc
     forRoom:room];
}
...
+ (void)kickOffNetworkConnection {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        CFStreamCreatePairWithSocketToHost(NULL, (CFStringRef) @"192.168.0.200" /*"linphone.org"*/,
...
static void showNetworkFlags(SCNetworkReachabilityFlags flags) {
    ...
    if (flags == 0) [log appendString:@"no flags."];
...
static void networkReachabilityNotification(CFNotificationCenterRef center, void *observer,
    ...
    if (newSSID != Nil && newSSID.length > 0 && mgr.SSID != Nil && newSSID.length > 0) {
        if (SCNetworkReachabilityGetFlags([mgr getProxyReachability], &flags)) {
            ...
            mgr.connectivity =
            none; // this will trigger a connectivity change in networkReachabilityCallback.
...
void networkReachabilityCallBack(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags,
                                 void *nilCtx) {
    ...
    if (theLinphoneCore != nil) {
        ...
        struct NetworkReachabilityContext *ctx =
        nilCtx ? ((struct NetworkReachabilityContext *)nilCtx) : 0;
        if ((flags == 0) || (flags & networkDownFlags)) {
            ...
            if (newConnectivity == wwan && proxy && isWifiOnly &&
                ...
                if (defaultExpire >= 0) linphone_proxy_config_expires(proxy, (int)defaultExpire);
            ...
            if (lm.connectivity != newConnectivity) {
                ...
                LOGI(@"Network connectivity changed to type [%s]",
                     (newConnectivity == wifi ? "wifi" : "wwan"));
                lm.connectivity = newConnectivity;
            }
        }
        if (ctx && ctx->networkStateChanged) { (*ctx->networkStateChanged)(lm.connectivity); }
    }
}
...
- (void)setupNetworkReachabilityCallback {
    ...
    if (proxyReachability) {
        ...
        SCNetworkReachabilityUnscheduleFromRunLoop(proxyReachability, CFRunLoopGetCurrent(),
                                                   kCFRunLoopDefaultMode);
        CFRelease(proxyReachability);
        proxyReachability = nil;
    }
    
    // This notification is used to detect SSID change (switch of Wifi network). The
    // ReachabilityCallback is not triggered when switching between 2 private Wifi... Since we
    // cannot be sure we were already observer, remove ourself each time... to be improved
    ...
    CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(),
                                    (__bridge const void *)(self), networkReachabilityNotification,
                                    CFSTR("com.apple.system.config.network_change"), NULL,
                                    CFNotificationSuspensionBehaviorDeliverImmediately);
    
    proxyReachability = SCNetworkReachabilityCreateWithAddress(
                                                               kCFAllocatorDefault, (const struct sockaddr *)&zeroAddress);
    
    if (!SCNetworkReachabilitySetCallback(
                                          proxyReachability, (SCNetworkReachabilityCallBack)networkReachabilityCallBack, ctx)) {
        LOGE(@"Cannot register reachability cb: %s", SCErrorString(SCError()));
        return;
    }
    if (!SCNetworkReachabilityScheduleWithRunLoop(proxyReachability, CFRunLoopGetCurrent(),
                                                  kCFRunLoopDefaultMode)) {
        LOGE(@"Cannot register schedule reachability cb: %s", SCErrorString(SCError()));
        return;
    }
    
    // this check is to know network connectivity right now without waiting for a change. Don'nt
    // remove it unless you have good reason. Jehan
    SCNetworkReachabilityFlags flags;
...
- (NetworkType)network {
    if ([[[UIDevice currentDevice] systemVersion] floatValue] < 7) {
        ...
        for (id subview in subviews) {
            if ([subview
                 isKindOfClass:[NSClassFromString(@"UIStatusBarDataNetworkItemView") class]]) {
...
- (void)finishCoreConfiguration {
    ...
    _contactSipField =
    [self lpConfigStringForKey:@"contact_im_type_value" inSection:@"sip" withDefault:@"SIP"];
    
    if (_fastAddressBook == nil) { _fastAddressBook = [[FastAddressBook alloc] init]; }
    ...
    if (camlist) {
        for (char *cam = *camlist; *camlist != NULL; cam = *++camlist) {
            ...
            if (strcmp(BACK_CAM_NAME, cam) == 0) { _backCamId = cam; }
        }
    } else {
    ...
    [self enableProxyPublish:([UIApplication sharedApplication].applicationState ==
                              UIApplicationStateActive)];
    ...
    // Post event
    NSDictionary *dict =
    [NSDictionary dictionaryWithObject:[NSValue valueWithPointer:theLinphoneCore]
                                forKey:@"core"];
...
- (void)startLinphoneCore {
    ...
    // - Security fix - remove multi transport migration, because it enables tcp or udp, if by
    // factoring settings only tls is enabled.     This is a problem for new installations.
    ...
    if (!bAudioInputAvailable) {
        UIAlertController *errView = [UIAlertController
                                      alertControllerWithTitle:NSLocalizedString(@"No microphone", nil)
                                      message:NSLocalizedString(@"You need to plug a microphone to your "
                                                                @"device to use the application.",
                                                                nil)
                                      preferredStyle:UIAlertControllerStyleAlert];
...
void popup_link_account_cb(LinphoneAccountCreator *creator, LinphoneAccountCreatorStatus status,
                           const char *resp) {
    if (status == LinphoneAccountCreatorStatusAccountLinked) {
        ...
        LinphoneProxyConfig *cfg = linphone_core_get_default_proxy_config(LC);
        if (cfg && strcmp(linphone_proxy_config_get_domain(cfg),
                          [LinphoneManager.instance lpConfigStringForKey:@"domain_name"
                                                               inSection:@"app"
                                                             withDefault:@"sip.linphone.org"]
                          .UTF8String) == 0) {
            UIAlertController *errView = [UIAlertController
                                          alertControllerWithTitle:NSLocalizedString(@"Link your account", nil)
                                          message:[NSString
                                                   stringWithFormat:
                                                   NSLocalizedString(@"Link your Linphone.org "
                                                                     @"account %s to your phone "
                                                                     @"number.",
                                                                     nil),
            ...
            [UIAlertAction actionWithTitle:NSLocalizedString(@"Maybe later", nil)
                                     style:UIAlertActionStyleDefault
                                   handler:^(UIAlertAction *action){}];
            
            UIAlertAction *continueAction = [UIAlertAction
                                             actionWithTitle:NSLocalizedString(@"Let's go", nil)
                                             style:UIAlertActionStyleDefault
                                             handler:^(UIAlertAction *action){
            ...
            [LinphoneManager.instance
             lpConfigSetInt:[[NSDate date] dateByAddingTimeInterval:
                             [LinphoneManager.instance
...
- (void)shouldPresentLinkPopup {
    NSDate *nextTime =
    [NSDate dateWithTimeIntervalSince1970:[self lpConfigIntForKey:@"must_link_account_time"
                                                      withDefault:1]];
    ...
    if (nextTime.timeIntervalSince1970 > 0 && [now earlierDate:nextTime] == nextTime) {
        ...
        if (cfg) {
            ...
            LinphoneAccountCreator *account_creator = linphone_account_creator_new(
                                                                                   LC, [LinphoneManager.instance lpConfigStringForKey:@"xmlrpc_url"
                                                                                                                            inSection:@"assistant"
                                                                                                                          withDefault:@""]
                                                                                   .UTF8String);
            ...
            linphone_account_creator_cbs_set_is_account_linked(
                                                               linphone_account_creator_get_callbacks(account_creator), popup_link_account_cb);
...
- (void)createLinphoneCore {
    ...
    .lastPathComponent;
    NSString *ringback =
    ([LinphoneManager bundleFile:[self lpConfigStringForKey:@"remote_ring" inSection:@"sound"]
      .lastPathComponent]
     ?: [LinphoneManager bundleFile:@"ringback.wav"])
    .lastPathComponent;
    NSString *hold =
    ([LinphoneManager bundleFile:[self lpConfigStringForKey:@"hold_music" inSection:@"sound"]
      .lastPathComponent]
     ?: [LinphoneManager bundleFile:@"hold.mkv"])
    .lastPathComponent;
    [self lpConfigSetString:[LinphoneManager bundleFile:ring]
                     forKey:@"local_ring"
                  inSection:@"sound"];
    [self lpConfigSetString:[LinphoneManager bundleFile:ringback]
                     forKey:@"remote_ring"
                  inSection:@"sound"];
    [self lpConfigSetString:[LinphoneManager bundleFile:hold]
                     forKey:@"hold_music"
                  inSection:@"sound"];
    ...
    linphone_core_cbs_set_message_received_unable_decrypt(
                                                          cbs, linphone_iphone_message_received_unable_decrypt);
     ...
     * server */
    linphone_core_set_root_ca(theLinphoneCore,
                              [LinphoneManager bundleFile:@"rootca.pem"].UTF8String);
    linphone_core_set_user_certificates_path(theLinphoneCore,
                                             [LinphoneManager cacheDirectory].UTF8String);
    
    /* The core will call the linphone_iphone_configuring_status_changed callback when the remote
     provisioning is loaded (or skipped). Wait for this to finish the code configuration */
    ...
    [NSNotificationCenter.defaultCenter
     ...
     object:nil];
    [NSNotificationCenter.defaultCenter addObserver:self
                                           selector:@selector(inappReady:)
                                               name:kIAPReady
                                             object:nil];
    
    /*call iterate once immediately in order to initiate background connections with sip server or
     * remote provisioning grab, if any */
    [self iterate];
    // start scheduler
...
- (void)destroyLinphoneCore {
    ...
    if (theLinphoneCore !=
        nil) { // just in case application terminate before linphone core initialization
        ...
        SCNetworkReachabilityUnscheduleFromRunLoop(proxyReachability, CFRunLoopGetCurrent(),
                                                   kCFRunLoopDefaultMode);
        if (proxyReachability) CFRelease(proxyReachability);
        proxyReachability = nil;
    }
    libStarted = FALSE;
    [[NSNotificationCenter defaultCenter] removeObserver:self];
...
- (LinphoneCall *)callByUsername:(NSString *)username {
    ...
    LinphoneAddress *linphoneAddress = linphone_core_interpret_url(
                                                                   [LinphoneManager getLc], [username cStringUsingEncoding:[NSString defaultCStringEncoding]]);
    if (linphoneAddress == NULL) { return NULL; }
...
- (LinphoneCall *)callByCallId:(NSString *)call_id {
    ...
    bctbx_list_t *call_tmp =
    bctbx_list_find_custom(calls, (bctbx_compare_func)comp_call_id, [call_id UTF8String]);
...
- (void)cancelLocalNotifTimerForCallId:(NSString *)callid {
    ...
    const bctbx_list_t *calls = linphone_core_get_calls(theLinphoneCore);
    bctbx_list_t *call =
    bctbx_list_find_custom(calls, (bctbx_compare_func)comp_call_id, [callid UTF8String]);
    if (call != NULL) {
        LinphoneCallAppData *data = (__bridge LinphoneCallAppData *)(linphone_call_get_user_data(
                                                                                                 (LinphoneCall *)call->data));
        if (data->timer) [data->timer invalidate];
        data->timer = nil;
        return;
    }
}
...
- (void)playMessageSound {
    ...
    if (!success) { LOGE(@"Could not play the message sound"); }
...
- (void)startPushLongRunningTask:(BOOL)msg callId:(NSString *)callId {
    ...
    if ([callId isEqualToString:@""]) return;
    ...
    if (msg) {
        ...
        [[UIApplication sharedApplication] beginBackgroundTaskWithExpirationHandler:^{
            if ([UIApplication sharedApplication].applicationState !=
                ...
                UNNotificationRequest *req =
                [UNNotificationRequest requestWithIdentifier:@"push_msg"
                                                     content:content
                                                     trigger:NULL];
                [[UNUserNotificationCenter currentNotificationCenter]
                 addNotificationRequest:req
            ...
            for (NSString *key in [LinphoneManager.instance.pushDict allKeys]) {
                [LinphoneManager.instance.pushDict setValue:[NSNumber numberWithInt:0]
                                                     forKey:key];
            }
            [[UIApplication sharedApplication] endBackgroundTask:pushBgTaskMsg];
            pushBgTaskMsg = 0;
        }];
        LOGI(@"Message long running task started for call-id [%@], remaining [%g s] because a push "
             @"has been received",
        ...
        [[UIApplication sharedApplication] beginBackgroundTaskWithExpirationHandler:^{
            if ([UIApplication sharedApplication].applicationState !=
                ...
                UNNotificationRequest *req =
                [UNNotificationRequest requestWithIdentifier:@"push_call"
                                                     content:content
                                                     trigger:NULL];
                [[UNUserNotificationCenter currentNotificationCenter]
                 addNotificationRequest:req
        ...
        LOGI(@"Call long running task started for call-id [%@], remaining [%g s] because a push "
             @"has been received",
             callId, [[UIApplication sharedApplication] backgroundTimeRemaining]);
    }
}
...
- (void)enableProxyPublish:(BOOL)enabled {
    ...
    if ([self lpConfigBoolForKey:@"publish_presence"]) {
        ...
        if (enabled) {
            linphone_core_set_presence_model(LC, linphone_core_create_presence_model_with_activity(
                                                                                                   LC, LinphonePresenceActivityTV, NULL));
        }
    ...
    while (lists) {
        linphone_friend_list_enable_subscriptions(
                                                  lists->data,
                                                  enabled && [LinphoneManager.instance lpConfigBoolForKey:@"use_rls_presence"]);
        lists = lists->next;
    }
}
...
- (void)refreshRegisters {
    if (connectivity == none) {
        // don't trust ios when he says there is no network. Create a new reachability context, the
        // previous one might be mis-functionning.
        LOGI(@"None connectivity");
        [self setupNetworkReachabilityCallback];
    }
    LOGI(@"Network reachability callback setup");
    linphone_core_refresh_registers(
                                    theLinphoneCore); // just to make sure REGISTRATION is up to date
}
...
- (void)copyDefaultSettings {
    ...
    if (IPAD && [[NSFileManager defaultManager] fileExistsAtPath:srcIpad]) { src = srcIpad; }
    NSString *dst = [LinphoneManager documentFile:@"linphonerc"];
...
- (void)acceptCall:(LinphoneCall *)call evenWithVideo:(BOOL)video {
    ...
    if ([self lpConfigBoolForKey:@"edge_opt_preference"]) {
        ...
        if (low_bandwidth) { LOGI(@"Low bandwidth mode"); }
...
- (void)send:(NSString *)replyText to:(NSString *)to {
    ...
    if (room) {
        ...
        if (linphone_core_lime_enabled(LC) == LinphoneLimeMandatory &&
            !linphone_chat_room_lime_available(room)) {
...
- (void)call:(const LinphoneAddress *)iaddr {
    ...
    if (!linphone_core_is_network_reachable(theLinphoneCore)) {
        UIAlertController *errView = [UIAlertController
                                      alertControllerWithTitle:NSLocalizedString(@"Network Error", nil)
                                      message:NSLocalizedString(@"There is no network connection available, "
                                                                @"enable WIFI or WWAN prior to place a "
                                                                @"call",
                                                                nil)
                                      preferredStyle:UIAlertControllerStyleAlert];
    ...
    if ([callCenter currentCalls] != nil &&
        ...
        UIAlertController *errView = [UIAlertController
                                      alertControllerWithTitle:NSLocalizedString(@"Cannot make call", nil)
    ...
    if (!iaddr) {
        UIAlertController *errView = [UIAlertController
                                      alertControllerWithTitle:NSLocalizedString(@"Invalid SIP address", nil)
                                      message:
                                      NSLocalizedString(
                                                        @"Either configure a SIP proxy server from settings prior to "
                                                        @"place a "
...
- (BOOL)doCall:(const LinphoneAddress *)iaddr {
    ...
    if (displayName != nil) { linphone_address_set_display_name(addr, displayName.UTF8String); }
    if ([LinphoneManager.instance lpConfigBoolForKey:@"override_domain_with_default_one"]) {
        linphone_address_set_domain(
                                    addr, [[LinphoneManager.instance lpConfigStringForKey:@"domain" inSection:@"assistant"]
                                           UTF8String]);
    }
    ...
    if (LinphoneManager.instance.nextCallIsTransfer) {
        ...
        if (call) {
            ...
            // - (void)onCall:StateChanged:withMessage:. If not, we are in big trouble and expect it
            // to crash We are NOT responsible for creating the AppData.
            ...
            if (data == nil) {
                ...
                /* will be used later to notify user if video was not activated because of the
                 * linphone core*/
            } else {
...
- (void)setPushNotificationToken:(NSData *)apushNotificationToken {
    if (apushNotificationToken == _pushNotificationToken) { return; }
...
#define APPMODE_SUFFIX @"prod"
        ...
        NSString *params =
        [NSString stringWithFormat:@"app-id=%@.voip.%@;pn-type=apple;pn-tok=%@;pn-msg-str=IM_"
         @"MSG;pn-call-str=IC_MSG;pn-"
         @"call-snd=%@;pn-msg-snd=msg.caf%@;pn-silent=1",
         [[NSBundle mainBundle] bundleIdentifier], APPMODE_SUFFIX,
         tokenString, ring, timeout];
        ...
        LOGI(@"Proxy config %s NOT configured for push notifications",
...
+ (NSString *)bundleFile:(NSString *)file {
    return [[NSBundle mainBundle] pathForResource:[file stringByDeletingPathExtension]
...
+ (int)unreadMessageCount {
    ...
    while (item) {
        ...
        if (room) { count += linphone_chat_room_get_unread_messages_count(room); }
        item = item->next;
    }
...
- (void)configureVbrCodecs {
    PayloadType *pt;
    int bitrate = lp_config_get_int(_configDb, "audio", "codec_bitrate_limit",
                                    kLinphoneAudioVbrCodecDefaultBitrate); /*default value is in
                                                                            linphonerc or
                                                                            linphonerc-factory*/
...
+ (id)getMessageAppDataForKey:(NSString *)key inMessage:(LinphoneChatMessage *)msg {
    ...
    if (appData) {
        NSDictionary *appDataDict = [NSJSONSerialization
                                     JSONObjectWithData:[NSData dataWithBytes:appData length:strlen(appData)]
                                     options:0
                                     error:nil];
...
+ (void)setValueInMessageAppData:(id)value
                          forKey:(NSString *)key
                       inMessage:(LinphoneChatMessage *)msg {
    
    NSMutableDictionary *appDataDict = [NSMutableDictionary dictionary];
    const char *appData              = linphone_chat_message_get_appdata(msg);
    if (appData) {
        appDataDict = [NSJSONSerialization
                       JSONObjectWithData:[NSData dataWithBytes:appData length:strlen(appData)]
...
- (void)lpConfigSetString:(NSString *)value forKey:(NSString *)key inSection:(NSString *)section {
    if (!key) return;
    lp_config_set_string(_configDb, [section UTF8String], [key UTF8String],
...
- (NSString *)lpConfigStringForKey:(NSString *)key withDefault:(NSString *)defaultValue {
    return [self lpConfigStringForKey:key
                            inSection:LINPHONERC_APPLICATION_KEY
...
- (int)lpConfigIntForKey:(NSString *)key withDefault:(int)defaultValue {
    return
    [self lpConfigIntForKey:key inSection:LINPHONERC_APPLICATION_KEY withDefault:defaultValue];
}
...
- (BOOL)lpConfigBoolForKey:(NSString *)key withDefault:(BOOL)defaultValue {
    return
    [self lpConfigBoolForKey:key inSection:LINPHONERC_APPLICATION_KEY withDefault:defaultValue];
}
...
+ (BOOL)isMyself:(const LinphoneAddress *)addr {
    ...
    while (it) {
        if (linphone_address_weak_equal(addr,
                                        linphone_proxy_config_get_identity_address(it->data))) {
            return YES;
        }
        it = it->next;
    }
...
// ugly hack to export symbol from liblinphone so that they are available for the linphoneTests
// target linphoneTests target do not link with liblinphone but instead dynamically link with
// ourself which is statically linked with liblinphone, so we must have exported required symbols
// from the library to have them available in linphoneTests DO NOT INVOKE THIS METHOD
